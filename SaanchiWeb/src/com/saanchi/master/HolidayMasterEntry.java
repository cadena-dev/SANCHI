...
public class HolidayMasterEntry extends ActionSupport implements ServletContextAware, ServletRequestAware {
	...
	private static final Logger log = Logger.getLogger(HolidayMasterEntry.class);
	...
	public String execute() throws Exception {
		...
		try {
			...
			log.fatal("At Holiday master Entry");
	...
	public String addHolidayMaster() throws Exception {
		...
		try {
			...
			HolidayMasterModel holiadyModel = new HolidayMasterModel();

			if(!manager.checkExistingHolidayDate(sdf1.parse(holidate_date))){
			...
			holiadyModel.setCreated_by(Integer.parseInt(empID));
			holiadyModel.setCreated_datetime(currentDate);
			holiadyModel.setIsactive(true);
		    holiadyModel.setHolidate_date(sdf1.parse(holidate_date));
			holiadyModel.setDescription(description);
			holiadyModel.setUpdated_by(Integer.parseInt(empID));
			holiadyModel.setUpdated_datetime(currentDate);

			operationFlag = manager.insertHolidayMaster(holiadyModel);
			}
	...
	@SkipValidation
	public String exitHolidayMaster() throws Exception {
		return "home";
	}
