...
public class UserMasterEdit extends ActionSupport implements ServletContextAware, ServletRequestAware {
	...
	private static final Logger log = Logger.getLogger(UserMasterEdit.class);
		...
		try {
			...
			officeList = manager.searchOfficeListForUser();
			 ...
			 activityList.add(optionVO);
				 ...
				 System.out.println("office_location "+this.office_location);
	...
	public String updateUserMaster() throws Exception {
		...
		try {
			...
			if(manager.checkExistingUserForEdit(Integer.parseInt(emp_no),user_name)) {

			UserCreateModel userModel = new UserCreateModel();

			userModel.setEmp_no(Integer.parseInt(emp_no));
			userModel.setIsactive(isactive.equalsIgnoreCase("true") ?true:false );
			...
			operationFlag = manager.updateUserMaster(userModel,"UPDATE");
			}
			else {
...
public String deleteUserMaster() throws Exception {
		...
		try {
			...
			userModel.setUpdated_by(Integer.parseInt(empID));
			userModel.setUpdated_datetime(currentDate);
			operationFlag = manager.updateUserMaster(userModel,"DELETE");
	...
	@SkipValidation
	public String exitUserMasterEdit() throws Exception {
		return "home";
	}
	...
	public void validate() {
		try {
			...
			System.out.println(user_type+" "+office_location);
